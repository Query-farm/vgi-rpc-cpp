...
arrow::Result<std::string> BindingHash(const std::string& name,
    ...
    for (const auto& [_, info] : methods) {
        ...
        entries.push_back(HashMethod{info.name,
                                     ...
                                     UnaryHasReturn(info), info.header_schema != nullptr,
                                     info.params_schema, info.result_schema, info.header_schema});
    }
...
arrow::Result<std::string> BuildServiceDescription(
    ...
    for (const MethodInfo* info : ordered) {
        ...
        ARROW_ASSIGN_OR_RAISE(auto result_ipc, UnaryHasReturn(*info)
                                                   ? SchemaIpc(info->result_schema)
    ...
    for (auto* b : std::vector<arrow::ArrayBuilder*>{&name_b, &type_b, &hasret_b, &hashdr_b,
                                                     &kind_b, &params_b, &result_b, &header_b,
                                                     &idem_b, &dep_b, &depmsg_b}) {
