...
std::shared_ptr<arrow::Array> require_column(const std::shared_ptr<arrow::RecordBatch>& batch,
...
auto get_value(const std::shared_ptr<arrow::RecordBatch>& batch, std::string_view name) {
...
template <typename ArrowArrayType>
auto get_optional_value(const std::shared_ptr<arrow::RecordBatch>& batch, std::string_view name)
...
std::vector<uint8_t> extract_binary(const std::shared_ptr<arrow::Array>& col,
                                    std::string_view name) {
    auto to_vec = [](auto view) {
        return std::vector<uint8_t>(reinterpret_cast<const uint8_t*>(view.data()),
...
// Optional variants
std::optional<std::string> get_optional_string(const std::shared_ptr<arrow::Array>& col,
                                               std::string_view name) {
    if (auto typed = std::dynamic_pointer_cast<arrow::StringArray>(col)) {
        if (typed->IsNull(0)) return std::nullopt;
        return typed->GetString(0);
...
std::optional<std::vector<uint8_t>> get_optional_binary(const std::shared_ptr<arrow::Array>& col,
                                                        std::string_view name) {
    auto to_vec = [](auto view) {
        return std::vector<uint8_t>(reinterpret_cast<const uint8_t*>(view.data()),
                                    reinterpret_cast<const uint8_t*>(view.data()) + view.size());
...
bool try_get_list_values(const std::shared_ptr<arrow::Array>& col, std::string_view name,
                         const char* expected_type, std::vector<T>& out) {
    ...
    if (!values)
        throw std::runtime_error(std::string("Type mismatch: expected list<") + expected_type +
                                 "> for parameter: " + std::string(name));
...
std::vector<T> get_list_values(const std::shared_ptr<arrow::Array>& col, std::string_view name,
                               const char* expected_type) {
    std::vector<T> result;
    if (try_get_list_values<arrow::ListArray, ValueArrayType, T>(col, name, expected_type, result))
        return result;
    if (try_get_list_values<arrow::LargeListArray, ValueArrayType, T>(col, name, expected_type,
                                                                      result))
        return result;
...
bool try_get_string_list_values(const std::shared_ptr<arrow::Array>& col, std::string_view name,
    ...
    throw std::runtime_error("Type mismatch: expected list<string> for parameter: " +
                             std::string(name));
}

// Try ListArray then LargeListArray for string lists
std::vector<std::string> get_string_list_values(const std::shared_ptr<arrow::Array>& col,
                                                std::string_view name) {
    std::vector<std::string> result;
    if (try_get_string_list_values<arrow::ListArray>(col, name, result)) return result;
    if (try_get_string_list_values<arrow::LargeListArray>(col, name, result)) return result;
    throw std::runtime_error("Type mismatch: expected list for parameter: " + std::string(name));
}

// Optional list extraction: returns nullopt on null instead of throwing.
template <typename ValueArrayType, typename T>
std::optional<std::vector<T>> get_optional_list_values(const std::shared_ptr<arrow::Array>& col,
                                                       std::string_view name,
                                                       const char* expected_type) {
    // Try ListArray
...
std::optional<std::vector<std::string>> get_optional_string_list_values(
    ...
    if (auto list_arr = std::dynamic_pointer_cast<arrow::ListArray>(col)) return extract(list_arr);
    if (auto list_arr = std::dynamic_pointer_cast<arrow::LargeListArray>(col))
...
std::vector<int64_t> Request::get<std::vector<int64_t>>(std::string_view name) const {
    return get_list_values<arrow::Int64Array, int64_t>(require_column(batch_, name), name, "int64");
}

template <>
std::vector<double> Request::get<std::vector<double>>(std::string_view name) const {
    return get_list_values<arrow::DoubleArray, double>(require_column(batch_, name), name,
                                                       "double");
}
