...
arrow::Result<std::string> TypeToken(const arrow::Field& field) {
  ...
  switch (type.id()) {
    ...
    case arrow::Type::LARGE_STRING:
      return std::string("large_utf8");
    case arrow::Type::STRING_VIEW:
      return std::string("utf8_view");
    case arrow::Type::BINARY:
      return std::string("binary");
    case arrow::Type::LARGE_BINARY:
      return std::string("large_binary");
    case arrow::Type::BINARY_VIEW:
      return std::string("binary_view");
    ...
    case arrow::Type::INTERVAL_MONTHS:
      return std::string("interval_months");
    case arrow::Type::INTERVAL_DAY_TIME:
      return std::string("interval_day_time");
    case arrow::Type::INTERVAL_MONTH_DAY_NANO:
      return std::string("interval_month_day_nano");
    case arrow::Type::DECIMAL128: {
      const auto& d = static_cast<const arrow::Decimal128Type&>(type);
      return "decimal128(" + std::to_string(d.precision()) + "," +
             std::to_string(d.scale()) + ")";
    }
    case arrow::Type::DECIMAL256: {
      const auto& d = static_cast<const arrow::Decimal256Type&>(type);
      return "decimal256(" + std::to_string(d.precision()) + "," +
             std::to_string(d.scale()) + ")";
    }
    case arrow::Type::LIST: {
      ARROW_ASSIGN_OR_RAISE(
          auto c, AnonChild(*static_cast<const arrow::ListType&>(type).value_field(), "item"));
      return "list<" + c + ">";
    }
