...
std::string compute_protocol_hash(
...
void register_describe(
    ...
    for (const auto& method_name : sorted_names) {
        ...
        auto result_schema =
            info.result_schema ? info.result_schema : empty_schema();
        auto result_buf = serialize_schema(result_schema);
    ...
    auto batch = arrow::RecordBatch::Make(
        ...
        {unwrap(name_builder.Finish()),
         unwrap(method_type_builder.Finish()),
         unwrap(has_return_builder.Finish()),
         unwrap(params_schema_builder.Finish()),
         unwrap(result_schema_builder.Finish()),
         unwrap(has_header_builder.Finish()),
         unwrap(header_schema_builder.Finish()),
         unwrap(is_exchange_builder.Finish())});
