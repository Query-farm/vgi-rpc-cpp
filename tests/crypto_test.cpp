...
TEST_CASE("sha256: FIPS 180-4 vectors", "[crypto]") {
    ...
    REQUIRE(abc.hex_digest() == "ba7816bf8f01cfea414140de5dae2223b00361a396177a9cb410ff61f20015ad");
}
...
TEST_CASE("aead: opens an envelope sealed by an independent implementation", "[crypto]") {
    ...
    const std::array<uint8_t, kAeadKeyBytes> key = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                                                    0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
                                                    0x10, 0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17,
                                                    0x18, 0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f};
    const uint8_t envelope[] = {
        0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c, 0x6d, 0x6e, 0x6f, 0x70, 0x71,
        0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7a, 0x7b, 0x0a, 0x14, 0x90, 0x9d,
        0x8d, 0x5e, 0xea, 0xa2, 0xe4, 0x42, 0xf2, 0x4c, 0xd5, 0x42, 0xde, 0xa1, 0x3e, 0xd8,
        0x39, 0x51, 0x94, 0x40, 0xab, 0xd9, 0xe2, 0x21, 0x01, 0xdf, 0x5f, 0x72, 0x58, 0x63,
        0xdb, 0x67, 0x2d, 0x10, 0x45, 0xd8, 0x90, 0xe7, 0x9a, 0x67, 0x16, 0x68, 0x74, 0x39,
        0x66, 0x34, 0x4e, 0xd3, 0xe4, 0x32, 0xfd, 0xcb, 0x40, 0xc3, 0xef, 0x6d, 0x67, 0x27,
        0x98, 0xe3, 0x9e, 0x0e, 0x39, 0xa6, 0xaa, 0x81, 0xb1, 0x1c, 0xdd, 0x03, 0x36, 0xde,
        0xe1, 0xd1, 0xd9, 0xa7, 0xdd, 0x88, 0x7e, 0x0a, 0xe7, 0xda, 0x45, 0x11, 0x1b, 0x40};
    const std::string aad = std::string("vgi_rpc.state.v4", 16) + std::string(
                                                                      "\x00\x01"
                                                                      "domain\x00"
                                                                      "principal",
                                                                      18);
    ...
    auto opened =
        aead_open(key, std::string(reinterpret_cast<const char*>(envelope), sizeof(envelope)), aad);
    REQUIRE(opened.has_value());
...
TEST_CASE("hex: round-trips and rejects non-hex", "[crypto]") {
    ...
    REQUIRE(bytes(hex_decode("000fffa5").value()) ==
            std::string(reinterpret_cast<const char*>(raw), 4));
